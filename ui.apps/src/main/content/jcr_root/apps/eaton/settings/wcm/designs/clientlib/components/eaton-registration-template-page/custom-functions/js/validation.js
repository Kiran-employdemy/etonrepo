...
function eatonValidateEmail(value) {
  var regex = RegExp(/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/i);
...
function eatonValidateUsCanPhone(value) {
  var regex = RegExp(/^[+]?1? ?(?:(?:\([2-9][0-9]{2}\))|[2-9][0-9]{2})[ \-]?[0-9]{3}[ \-]?[0-9]{4}$/);
...
function eatonValidateZipPostal(value, countrySom) {
  var usPattern = /^[0-9]{5}(?:[-\s+]?[0-9]{4})?$/;
  var caPattern = /^([A-Za-z]\d[A-Za-z][-\s]?\d[A-Za-z]\d)$/;

  var countryVal = guideBridge.resolveNode(countrySom).value;
  var pattern = void 0;
...
function eatonLookupEmail(emailAddressSom, messageSom) {
  ...
  var email = guideBridge.resolveNode(emailAddressSom);
  var messageElem = guideBridge.resolveNode(messageSom);
  var validationResult = false;

  // Validating value of email field
  if (email.value) {
    var emailValid = eatonValidateEmail(email.value);
    if (emailValid) {
  ...
  function emailCheck() {
    var emailAdd = document.querySelector('[aria-label="Email"]').value;
    var comfirmAdd = document.querySelector('[aria-label="Confirm Email"]').value;
...
function eatonLookupSupplier(supplierNumberSom, messageSom) {
  // initializing variables
  var supplierNumber = guideBridge.resolveNode(supplierNumberSom);
  var messageElem = guideBridge.resolveNode(messageSom);
  var validationResult = false;
...
var resetPanelFields = function resetPanelFields() {
