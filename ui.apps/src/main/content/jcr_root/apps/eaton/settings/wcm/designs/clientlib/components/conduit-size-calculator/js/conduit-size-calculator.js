...
$(document).ready(function () {
  $('select').niceSelect();
  var calBtn = $('#souriau-toolkit-counduit-size-calc-main-form');
  var errorPane = $('.u-error');
  var successPane = $('.u-success');
  var errorClose = $('.u-error-close');
  var successClose = $('.u-success-close');
  var calData = $('#calculator');
  var calTitle = $('#calc-title');
  ...
  function souriauToolkitConduitSizeCalcMainCalculator(values) {
    ...
    var fillRate = values.fillRate;
    var numWires = values.rows.numWires;
    var wireDiameter = values.rows.wireDiameter;
    var totalWires = 0;
    var averageDiameter = 0;
    var numericValidated = false;
    var setError = false;

    // Check if wires are empty
    if (Object.values(numWires).every(function (x) {
      return !x || x === null || x === '';
    })) {
      setError = true;
      ...
      Object.entries(numWires).forEach(function (_ref) {
      ...
      if (totalWires && !numericValidated) {
        ...
        Object.entries(wireDiameter).forEach(function (_ref3) {
        ...
        if (!numericValidated) {
          ...
          var factored = souriauToolkitConduitSizeCalcGetFactored(Number(totalWires), Number(averageDiameter));
          if (factored === 'FALSE') {
            setError = true;
  ...
  function souriauToolkitConduitSizeCalcGetConduitSize(fillRate, factored) {
    var conduitSize = null;
    var getStandardFactor = souriauToolkitConduitSizeCalcGetStandardFactored(fillRate);
    var flag = true;
    Object.entries(getStandardFactor).forEach(function (_ref5) {
  ...
  function souriauToolkitCounduitSizeCalcGetStandard(key) {
    var conduitSizeStandard = ['06 - 3/16', '09 - 9/32', '10 - 5/1', '12 - 3/8', '14 - 7/16', '16 - 1/2', '20 - 5/8', '24 - 3/4', '28 - 7/8', '32 - 1', '40 - 1-1/4', '48 - 1-1/2', '56 - 1-3/4', '64 - 2.000'];
  ...
  function calculateValues(values) {
    var valuesToCalculate = values && values.length ? values[0] : {};
  ...
  function getValuesFromTable() {
    var getValuesFromTable = $('#souriau-toolkit-counduit-size-calc-main-form').serializeArray().reduce(function (obj, item) {
      obj[item.name] = item.value;
      return obj;
    }, {});

    var formatedValues = formatTableValues(getValuesFromTable);
    var calculatedResult = calculateValues(formatedValues);
