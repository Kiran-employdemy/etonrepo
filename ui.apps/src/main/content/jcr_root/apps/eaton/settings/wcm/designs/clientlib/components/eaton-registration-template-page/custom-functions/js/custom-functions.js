...
function eatonGetCountryCookie() {
  var countryID = document.cookie.replace(/(?:(?:^|.*;\s*)etn_country_selector_country\s*\=\s*([^;]*).*$)|^.*$/, '$1');

  // Need to return value that will be acceptable to AF
  var countryWords = countryID.toLowerCase().split('-');
...
function eatonIsMultipleChecked(formElementSom) {
  var formElement = guideBridge.resolveNode(formElementSom);
...
function eatonBuildDynamicOptions(updatedElementSom, targetElementSom) {
  var updatedElement = guideBridge.resolveNode(updatedElementSom);
  var targetElement = guideBridge.resolveNode(targetElementSom);
  var $updatedElement = $('.' + updatedElement.name);
  ...
  if (!targetElement.value) {
    ...
    var dynamicItems = $updatedElement.prev().data('dynamicItems');

    var itemsArray = [];

    for (var i = 0; i < dynamicItems.length; i++) {
      var dynamicItem = dynamicItems[i];
 ...
 */
function eatonOptionsForField(fieldSom, parameterName, keyName, valueName, sortKey, keyValueSeparator) {

  var field = guideBridge.resolveNode(fieldSom);
  var prefillValue = field.value ? field.value : '';
...
function calculateFieldOptions(parameterName, keyName, valueName, sortKey, keyValueSeparator) {
  var options = guideBridge.eatonFields[parameterName];
  ...
  options = options.map(function (option) {
    var optionString = option[keyName] + '=';
...
function eatonGetStatesForCountry(countrySom, stateSom) {
  var countryField = guideBridge.resolveNode(countrySom);
  var stateField = guideBridge.resolveNode(stateSom);

  var countryVal = countryField.value;
  // Get the country based on the countryCode set
  var country = guideBridge.eatonFields.validCountries.filter(function (countryItem) {
    return countryItem.countryCode === countryVal;
  })[0];

  var countryId = country.countryId;

  // Filter States with Country ID
  var options = guideBridge.eatonFields.validStates.filter(function (stateItem) {
    return stateItem.countryId === countryId;
  });

  options = options.sort(propComparator('stateName')).map(function (countryItem) {
...
function eatonFormatRepeatablePanels(dataFieldSom) {
  var dataField = guideBridge.resolveNode(dataFieldSom);
  var result = {};

  for (var i = 0; i < guideBridge.etnRepeatablePanels.length; i++) {
    var repeatablePanel = guideBridge.resolveNode(guideBridge.etnRepeatablePanels[i]);
    var refName = getProperBindRef(repeatablePanel);

    if (refName && repeatablePanel.instanceManager.instances.length) {
      result[refName] = [];
      for (var j = 0; j < repeatablePanel.instanceManager.instances.length; j++) {
        var panel = repeatablePanel.instanceManager.instances[j];
 ...
 */
function eatonSetValueBasedOnField(watchFieldSom, setFieldSom, watchValue, setValue) {
  var setField = guideBridge.resolveNode(setFieldSom);

  guideBridge.on('elementValueChanged', function (e, payload) {
    if (payload.target.somExpression === watchFieldSom) {
      var value = guideBridge.resolveNode(watchFieldSom).value;
      var matched = isValueMatched(watchValue, value);
...
function propComparator(propName) {
  return function (a, b) {
    var nameA = a[propName];
    var nameB = b[propName];
...
function iterateFields(field, maxDepth, depth, result) {
  ...
  if (field.children && depth <= maxDepth) {
    for (var i = 0; i < field.children.length; i++) {
      var subField = field.children[i];
      if (subField.className !== 'guideInstanceManager' && subField.className !== 'guideButton') {
        var refName = getProperBindRef(subField);
...
function isValueMatched(checkedValue, actualValue) {
  ...
  var values = actualValue.split(splitChar);
