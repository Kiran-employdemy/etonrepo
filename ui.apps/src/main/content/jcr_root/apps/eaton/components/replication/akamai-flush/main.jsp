...
%><%@page session="false" contentType="text/html" pageEncoding="utf-8"
    ...
    final boolean generalError =
            StringUtils.equals("/akamai-error", slingRequest.getRequestPathInfo().getSuffix());
%>
...
<div class="call-out">
    <p>
        Your akamai flush requests have been issued with the following results:
    </p>

    <% if(generalError) { %>
    <p>
        An error occurred during clearing akamai cache.
    </p>
    <% } %>
    ...
    <ul>
        ...
        if(index == 0 && !generalError) { %>No active Akamai Flush agents could be found for this run mode.<% } %>
    </ul>

    <% if(errors || generalError) { %>
    <p>
        Please review your Akamai Flush Agent logs to ensure all replication requests were successfully processed.
    </p>
    <% } %>
</div>
<% } %>
