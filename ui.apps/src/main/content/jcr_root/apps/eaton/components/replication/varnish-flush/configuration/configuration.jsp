...
%><%@page session="false" contentType="text/html" pageEncoding="utf-8"
    ...
    for(Map.Entry<String, Agent> entryAgent : agents.entrySet()) {
        Agent agentData = entryAgent.getValue();
         ...
         String domainName = PropertiesUtil.toString(propertyConfig.get("varnishDomain"), StringUtils.EMPTY);
...
<form id="varnishconfig" method="POST">
    <input class="button" type="button"  value="Flush Paths on Varnish"/>
</form>
<% } %>
...
<h3>Active Varnish Flush Agents (Includes only Varnish Agents)</h3>
<ul>
    ...
    Varnish Flush replication agents</a></li><% } %>
    <% for(Map.Entry<String, Agent> entry : agents.entrySet()) {
           ...
           String domain = PropertiesUtil.toString(property.get("varnishDomain"), StringUtils.EMPTY);
...
<script>
    ...
    $("#varnishconfig").click(function(e)
    {
        $.ajax({
            url: '/eaton/clearVarnishCache.json',
            type: 'POST',
            data:  {
                "path":'${resource.path}',
            ...
            success: function(data, textStatus, jqXHR)
            {
                 if (data.hasOwnProperty("redirectPath")) {
                     ...
                     CQ.Ext.Msg.alert('Status',"An error occurred during clearing varnish cache. Response Code: "+jqXHR.status);
                 }
            },
