...
boolean isLazyLoad = cfg.get("lazyLoad", false);
    ...
    if (isComposite) {
        ...
        if (contentResource != null) {
            for (Resource item : contentResource.getChildren()) {
                if(isLazyLoad){
                    %><div class="lazyload" data-lazyload= '<coral-multifield-item><% include(field, item, cmp, slingRequest,log); %></coral-multifield-item>'></div><%
                } else { %><coral-multifield-item><% include(field, item, cmp, slingRequest,log); %></coral-multifield-item><%
               }
            }
        }
    } else {
 ...
 */
@SuppressWarnings({ "deprecation", "null" })
private static void include(Resource field, Resource item, ComponentHelper cmp, SlingHttpServletRequest request,Logger log) throws Exception {
    ...
    request.setAttribute(Value.FORM_VALUESS_ATTRIBUTE, existingVM);
    request.setAttribute(Value.CONTENTPATH_ATTRIBUTE, existingPath);
    log.info(Value.FORM_VALUESS_ATTRIBUTE+"::::::::"+existingVM);
    log.info(Value.CONTENTPATH_ATTRIBUTE+"::::::::"+existingPath);
}
