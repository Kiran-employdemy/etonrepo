...
/*
    Run this script for the list of paths gotten from the output of the report generated with all references to the tag with name oldValue.

    When selecting verbose you will get a detail of what has been changed from to where.
    ...
    Example of data:
    {
        "verbose":true,
        "dryRun":true,
        "oldValue":"eaton:product-taxonomy/control-drives-automation-sensors/lighting-contactors/c30cn-lighting-contactors",
        "newValue":"eaton:product-taxonomy/control-drives-automation-sensors/contactors-and-starters/c30cn-lighting-contactors",
...
try {
    paths.split(',').each { path ->
        if (!session.nodeExists(path)) {
            ...
            performTagMoveOnNode(session, node)
            augmentCounter(session, node)
        }
    }
    println '*************************************************'
    println dryRun ? "The tag ${oldValue} would have been changed to ${newValue} on :" : "The tag ${oldValue} has been changed to ${newValue} on:"
...
def performTagMoveOnNode(session, node) {
    node.getProperties().each { property ->
        if (!property.name.equals("cq:tags")) {
            if (!property?.isMultiple()) {
                handleSingleValue(node, property)
            } else {
                String[] originalValues = property?.getValues() as String[]
                handleMultipleValue(node, originalValues, property)
            }
            if (!dryRun) {
                session.save()
            }
...
private void handleMultipleValue(node, originalValues, property) {
    printLineIfVerbose("Value of ${property.getName()} is multiple")
    if (originalValues.find({ it == oldValue }) != null) {
        printLineIfVerbose("The values contain ${oldValue}, ${dryRun? 'changing' : ' would have been changed '} to ${newValue}")
        originalValues -= oldValue;
        originalValues += newValue;
        node.setProperty(property.name, originalValues as String[]);
        println "The property ${property.name} ${dryRun ? ' would have been changed ' : ' is changed '} on ${node.path} to ${node.getProperty(property.name).getValues() as String []}"
    }
}

private void handleSingleValue(node, property) {
    printLineIfVerbose("Value of ${property.getName()} is single")
    String stringProperty = property.getValue() as String
    if (stringProperty.contains(oldValue)) {
        printLineIfVerbose("The value contains ${oldValue}, changing to ${newValue}")
        node.setProperty(property.name, stringProperty.replace(oldValue, newValue))
        printLineIfVerbose("Property ${property.name} ${dryRun ? ' would have been changed ' : ' is changed '} on ${node.path} to  ${node.getProperty(property.name).getValue() as String}")
    }
}

private def isPage(session, node) {
    def typePage = getResourcePath(node, session).primaryNodeType.isNodeType("cq:Page")
    return typePage
}

private def getResourcePath(node, session) {
    def path = node.path
...
private def isAsset(session, node) {
