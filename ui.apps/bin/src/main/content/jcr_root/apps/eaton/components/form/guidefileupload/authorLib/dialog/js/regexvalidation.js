(function ($, $document) {
    ...
    $(document).on("click", ".cq-dialog-submit", function (e) {
        var regexSelector = $('[data-regex]').not(".richtext-container>div.coral-RichText-editable");
        var error = false;
        var invalidTabs = [];
        var errorMsg = "";
        ...
        if (regContentHeading === null || typeof regContentHeading === "undefined") {
            regContentHeading = "Following Field is invalid";
        }
        ...
        function validateField($this, value, regex, regexText, dialogMode) {
            ...
            if ((!(new RegExp(regex).test(value)) && regex != "required") || (regex === "required" && value.length == 0)) {
                error = true;
                if ($this.hasClass('coral-PathBrowser') || $this.hasClass('coral-ColorInput') || $this.hasClass('coral3-ColorInput') || $this.prop('tagName').toLowerCase() === "coral-datepicker" || $this.prop('tagName').toLowerCase() === "foundation-autocomplete") {
                    $this.find("input.coral3-Textfield").addClass("is-invalid").addClass("regex-invalid");
		    $this.find("input.coral-Textfield").addClass("is-invalid").addClass("regex-invalid");
		    ...
		    $this.find("input.coral3-Textfield").addClass("is-invalid").addClass("regex-invalid");
                ...
                if (dialogMode != 1) {
                    if ($this.parent().hasClass("richtext-container")) {
                        $this.siblings(".coral-RichText-editable").addClass("regex-invalid");
                ...
                if ($this.hasClass('coral-PathBrowser') || $this.hasClass('coral-ColorInput') || $this.hasClass('coral3-ColorInput') || $this.prop('tagName').toLowerCase() === "coral-datepicker" || $this.prop('tagName').toLowerCase() === "foundation-autocomplete") {
                    $this.find("input.coral-Textfield").removeClass("is-invalid").removeClass("regex-invalid");
		    ...
		    $this.find("input.coral3-Textfield").removeClass("is-invalid").removeClass("regex-invalid");
                ...
                if ($this.parent().hasClass("richtext-container")) {
                    $this.siblings(".coral-RichText-editable").removeClass("regex-invalid");
        ...
        function getValue(regex, value, $inputEle) {
	    	...
	    	if($inputEle.find("input.coral-Textfield").length>0)
                	value = $inputEle.find("input.coral-Textfield").attr("aria-valuenow");
		else
			value = $inputEle.find("input.coral3-Textfield").attr("aria-valuenow");
                ...
                if($inputEle.find("input.coral-Textfield").length>0)
			value = $inputEle.find("input.coral-Textfield").val();
		else
            ...
            } else if ($inputEle.hasClass("coral-Checkbox") || $inputEle.hasClass("coral3-Checkbox")) {
