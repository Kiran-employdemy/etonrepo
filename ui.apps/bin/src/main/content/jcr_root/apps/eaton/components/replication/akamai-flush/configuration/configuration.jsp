...
%><%@page session="false" contentType="text/html" pageEncoding="utf-8"
    ...
    for(Map.Entry<String, Agent> entryAgent : agents.entrySet()) {
        Agent agentData = entryAgent.getValue();
         ...
         String domainName = PropertiesUtil.toString(propertyConfig.get("domain"), StringUtils.EMPTY);
...
<form id="akamaiconfig" method="POST">
    <input class="button" type="button"  value="Flush Paths on Akamai"/>
</form>
<% } %>
...
<ul>
    <% if(!hasPaths) { %><li class="not-set">Akamai flush paths not set</li><% } %>
    <% for(final String path : paths) { %>
    <li><%= path %></li>
    <% } %>
</ul>

<h3>Active Akamai Flush Agents (Includes only Akamai Agents)</h3>
<ul>
    ...
    Akamai Flush replication agents</a></li><% } %>
    <% for(Map.Entry<String, Agent> entry : agents.entrySet()) {
           ...
           String domain = PropertiesUtil.toString(property.get("domain"), StringUtils.EMPTY);
...
<script>
    ...
    $("#akamaiconfig").click(function(e)
    {
        $.ajax({
            url: '/eaton/clearAkamaiCache.json',
            type: 'POST',
            data:  {
                "path":'${resource.path}',
            ...
            success: function(data, textStatus, jqXHR)
            {
                 if (data.hasOwnProperty("redirectPath")) {
                     ...
                     CQ.Ext.Msg.alert('Status',"An error occurred during clearing akamai cache. Response Code: "+jqXHR.status);
                 }
            },
