...
class ProductSecureAssetValidatorTest {
    ...
    void testCorrectValidateMethodIsCalled() {
        when(userProfile.getProductCategoriesTags()).thenReturn(LIST_OF_TAGS);
        when(secureAssetModel.validateProductCategoriesTags(LIST_OF_TAGS)).thenReturn(false);

        assertFalse(new ProductSecureAssetValidator(userProfile).validate(secureAssetModel), "must return value of method call on secureAssetModel");
    }
