...
/**
 * The Class FindReplaceTagsDataPageServiceImpl.
 *
 * Service that executes "find and replace" functionality for tags for pages.
 *
...
@Component(
		...
		property = {
				...
				AEMConstants.SERVICE_DESCRIPTION + "Find Replace Tags Data Page Service",
				AEMConstants.PROCESS_LABEL + "FindReplaceTagsDataPageServiceImpl" })
public class FindReplaceTagsDataPageServiceImpl extends AbstractFindReplaceTagsDataTypeService implements FindReplaceTagsDataTypeService {

	private static final Logger LOGGER = LoggerFactory.getLogger(FindReplaceTagsDataPageServiceImpl.class);

	private static final ContentType CONTENT_TYPE = ContentType.PAGE;
	...
	public TagsModificationResultBean add(final TagsModificationRequestBean modificationRequest) {
		return this.processResources(modificationRequest, this.addProcessor, "An error has occurred while adding tags to pages.");
	}
	...
	public TagsModificationResultBean delete(final TagsModificationRequestBean modificationRequest) {
		return this.processResources(modificationRequest, this.deleteProcessor, "An error has occurred while deleting tags in pages.");
	}
	...
	public TagsModificationResultBean replace(final TagsModificationRequestBean modificationRequest) {
		return this.processResources(modificationRequest, this.replaceProcessor, "An error has occurred while replacing tags in pages.");
	}
	...
	protected String getSearchQuery(final TagsSearchRequestBean searchRequest) {
		...
		for (final Map.Entry<String, List<String>> entry : entrySet) {
			...
			for (final String tagValue : tagValueList) {
				tagsQuery.append("(");
				tagsQuery.append(StrSubstitutor.replace(FindAndReplaceConstants.PAGE_QUERY_TAG, Map.of(FindAndReplaceConstants.PROP_NAME_PARAM, propertyName,
		...
		return StrSubstitutor.replace(FindAndReplaceConstants.QUERY, Map.of("nodeType", NameConstants.NT_PAGE, "pathToSearch", searchRequest.getPath(),
